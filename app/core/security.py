...
async def verify_cafe_access(cafe_id: str, current_user: Dict[str, Any]) -> bool:
    """Verify if current user has access to cafe"""
    ...
    # Admin can access all cafes
    if current_user.get("role") == "admin":
        return True
    
    # Cafe owner can only access their own cafes
    if not verify_cafe_ownership(cafe["owner_id"], current_user["id"]):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
